...
BOOL CQR_Encode::EncodeData(int nLevel, int nVersion, BOOL bAutoExtent, int nMaskingNo,const char* lpsSource, int ncSource)
{
	...
	int i;for ( i = (m_ncDataCodeWordBit + 7) / 8; i < ncDataCodeWord; ++i)
	{
...
{
	int nVersionGroup = nVersion >= 27 ? QR_VRESION_L : (nVersion >= 10 ? QR_VRESION_M : QR_VRESION_S);

	int i;for ( i = nVersionGroup; i <= QR_VRESION_L; ++i)
	{
...
BOOL CQR_Encode::EncodeSourceData(LPCSTR lpsSource, int ncLength, int nVersionGroup)
{
	// 初期データモード取得
	int nDataMode = GetInitDataMode(lpsSource, ncLength, nVersionGroup); 
	...
	while (ncComplete < ncLength && m_ncDataCodeWordBit != -1)
	{
		int ncContinue = 0; // モード継続文字数

		if (nDataMode == QR_MODE_8BIT)
		{
			/////////////////////////////////////////////////////////////////
			// ８ビットバイトモード

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 4, 4); // ８ビットバイトモードインジケータ(0100b)

			int i;for ( i = ncComplete; i < ncLength; ++i)
			{
				// モード継続文字数、モード切替チェック
				...
				{
					// 漢字発生 → 漢字モード
					nDataMode = QR_MODE_KANJI;
					break;
				}

				static int nCount1[] = {6, 8, 9};

				if (GetContinueCount(lpsSource + i, ncLength - i, QR_MODE_NUMERAL) >= nCount1[nVersionGroup])
				{
					// {6, 8, 9}以上の数字列が発生 → 数字モード
					nDataMode = QR_MODE_NUMERAL;
					break;
				}

				static int nCount2[] = {11, 15, 16};

				if (GetContinueCount(lpsSource + i, ncLength - i, QR_MODE_ALPHABET) >= nCount2[nVersionGroup])
				{
					// {11, 15, 16}以上の英数字の排他的部分集合の列が発生 → 英数字モード
					nDataMode = QR_MODE_ALPHABET;
					break;
				}
			...
			// 文字数セット
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (WORD)ncContinue, nIndicatorLen8Bit[nVersionGroup]);

			// ８ビットバイトモードでビット列保存
			for (i = 0; i < ncContinue; ++i)
			{
				m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (WORD)lpsSource[ncComplete + i], 8);
			}

			ncComplete += ncContinue;
		}

		else if (nDataMode == QR_MODE_ALPHABET)
		{
			/////////////////////////////////////////////////////////////////
			// 英数字モード

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 2, 4); // 英数字モードインジケータ(0010b)
			...
			int i;for ( i = ncComplete; i < ncLength; ++i)
			{
				...
				if (! IsAlphabetData(lpsSource[i]))
				{
					// ８ビットバイト文字集合の排他的部分集合が発生 → ８ビットバイトモード
					nDataMode = QR_MODE_8BIT;
					break;
				}

				static int nCount[] = {13, 15, 17};

				if (GetContinueCount(lpsSource + i, ncLength - i, QR_MODE_NUMERAL) >= nCount[nVersionGroup])
				{
					// {13, 15, 17}以上の数字列が発生 → 数字モード
					nDataMode = QR_MODE_NUMERAL;
					break;
				}
			...
			// 文字数セット
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (WORD)ncContinue, nIndicatorLenAlphabet[nVersionGroup]);

			// 英数字モードでビット列保存
			for (i = 0; i < ncContinue; i += 2)
			{
				WORD wBinCode;

				if (i < ncContinue - 1)
				{
					wBinCode = (WORD)((AlphabetToBinaly(lpsSource[ncComplete + i]) * 45) +
									   AlphabetToBinaly(lpsSource[ncComplete + i + 1]));
				...
				{
					// 最終１バイト
					wBinCode = (WORD)AlphabetToBinaly(lpsSource[ncComplete + i]);
		...
		else if (nDataMode == QR_MODE_NUMERAL)
		{
			/////////////////////////////////////////////////////////////////
			// 数字モード

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 1, 4); // 数字モードインジケータ(0001b)
			...
			int i;for ( i = ncComplete; i < ncLength; ++i)
			{
				...
				if (IsAlphabetData(lpsSource[i]) && ! IsNumeralData(lpsSource[i]))
				{
					// 英数字集合の排他的部分集合が発生 → 英数字モード
					nDataMode = QR_MODE_ALPHABET;
					break;
				}
			...
			// 文字数セット
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (WORD)ncContinue, nIndicatorLenNumeral[nVersionGroup]);

			// 数字モードでビット列保存
			for (i = 0; i < ncContinue; i += 3)
			{
				WORD wBinCode;

				if (i < ncContinue - 2)
				{
					wBinCode = (WORD)(((lpsSource[ncComplete + i] - '0') * 100) +
									  ((lpsSource[ncComplete + i + 1] - '0') * 10) +
									   (lpsSource[ncComplete + i + 2] - '0'));

					m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, wBinCode, 10);
				}
				else if (i == ncContinue - 2)
				{
					// 最終２バイト
					wBinCode = (WORD)(((lpsSource[ncComplete + i] - '0') * 10) +
									   (lpsSource[ncComplete + i + 1] - '0'));

					m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, wBinCode, 7);
				}
				else if (i == ncContinue - 1)
				{
					// 最終１バイト
					wBinCode = (WORD)(lpsSource[ncComplete + i] - '0');
		...
		else if (nDataMode == QR_MODE_KANJI)
		{
			/////////////////////////////////////////////////////////////////
			// 漢字モード

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 8, 4); // 漢字モードインジケータ(1000b)

			ncContinue = GetContinueCount(lpsSource + ncComplete, ncLength - ncComplete, QR_MODE_KANJI);

			// 文字数セット
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (WORD)(ncContinue / 2), nIndicatorLenKanji[nVersionGroup]);

			// 漢字モードでビット列保存
			int i;for ( i = 0; i < ncContinue; i += 2)
			{
				WORD wBinCode = KanjiToBinaly((WORD)(((BYTE)lpsSource[ncComplete + i] << 8) + (BYTE)lpsSource[ncComplete + i + 1]));
		...
		if (ncComplete < ncLength)
		{
			nDataMode = GetInitDataMode(lpsSource + ncComplete, ncLength - ncComplete, nVersionGroup); 
		}
	}
...
/////////////////////////////////////////////////////////////////////////////
// CQR_Encode::GetInitDataMode
// 用  途：初期入力データのモード取得
// 引  数：入力データ、入力データ長
// 戻り値：モード
// 備  考：漢字モードから他のモード変遷時にも使用

int CQR_Encode::GetInitDataMode(LPCSTR lpsSource, int ncLength, int nVersionGroup)
{
	if (ncLength >= 2 && IsKanjiData(lpsSource[0], lpsSource[1]))
	{
		// 初期入力データが漢字 → 漢字モード
		return QR_MODE_KANJI;
	}

	if (! IsAlphabetData(lpsSource[0]))
	{
		// 初期入力データが８ビットバイト文字の排他的部分集合 → ８ビットバイトモード
		return QR_MODE_8BIT;
	}
	else
	{
		if (! IsNumeralData(lpsSource[0]))
		{
			// 初期入力データが英数字の排他的文字集合

			static int nCount[] = {6, 7, 8};

			int i;for ( i = 0; i < min(nCount[nVersionGroup], ncLength) ; ++i)
			{
				if (! IsAlphabetData(lpsSource[i]))
				{
					// {6, 7, 8}より少ない文字数でかつ８ビットバイト文字が継続 → ８ビットバイトモード
					return QR_MODE_8BIT;
				}
			}

			// 英数字モード
			return QR_MODE_ALPHABET;
		}
		else
		{
			// 初期入力データが数字
			static int nCount1[] = {4, 4, 5};

			int i;for ( i = 0; i < min(nCount1[nVersionGroup], ncLength) ; ++i)
			{
				if (! IsAlphabetData(lpsSource[i]))
				{
					// {4, 4, 5}より少ない文字数でかつ８ビットバイトの排他的サブセットのデータが継続 → ８ビットバイトモード
					return QR_MODE_8BIT;
				}
			}

			static int nCount2[] = {7, 8, 9};

			for (i = 0; i < min(nCount2[nVersionGroup], ncLength) ; ++i)
			{
				if (IsAlphabetData(lpsSource[i]) && ! IsNumeralData(lpsSource[i]))
				{
					// {7, 8, 9}より少ない文字数で英数字の排他的部分集合が継続 → 英数字モード
					return QR_MODE_ALPHABET;
				}
			}

			// 数字モード
			return QR_MODE_NUMERAL;
		}
	}
}
...
/////////////////////////////////////////////////////////////////////////////
// CQR_Encode::GetContinueCount
// 用  途：モード継続文字数取得
// 引  数：入力データ、入力データ長、モード
// 戻り値：継続文字数（漢字は１文字で２）

int CQR_Encode::GetContinueCount(LPCSTR lpsSource, int ncLength, int nDataMode)
{
	int ncContineu = 0;

	int i;for ( i = 0; i < ncLength; ++i)
	{
		if (nDataMode == QR_MODE_KANJI)
		{
			if (i < ncLength - 1 && IsKanjiData(lpsSource[i], lpsSource[i + 1]))
			{
				ncContineu += 2;
...
{
	int i;for ( i = 0; i < ncDataCodeWord ; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			BYTE nExpFirst = byIntToExp[lpbyRSWork[0]]; // 初項係数より乗数算出

			int j;for ( j = 0; j < ncRSCodeWord; ++j)
			{
			...
			// 残り桁をシフト
			for (j = ncRSCodeWord; j < ncDataCodeWord; ++j)
				lpbyRSWork[j] = lpbyRSWork[j + 1];
		}
		else
		{
			// 残り桁をシフト
			int j;for ( j = 0; j < ncDataCodeWord; ++j)
				lpbyRSWork[j] = lpbyRSWork[j + 1];
		}
	}
}
...
void CQR_Encode::FormatModule()
{
	...
	// モジュールパターンをブール値に変換
	int i;for ( i = 0; i < m_nSymbleSize; ++i)
	{
		int j;for ( j = 0; j < m_nSymbleSize; ++j)
		{
...
void CQR_Encode::SetFunctionModule()
{
	...
	for (i = 0; i < QR_VersonInfo[m_nVersion].ncAlignPoint; ++i)
	{
		...
		int j;for ( j = 0; j < QR_VersonInfo[m_nVersion].ncAlignPoint; ++j)
		{
...
void CQR_Encode::SetCodeWordPattern()
{
	...
	int i;for ( i = 0; i < m_ncAllCodeWord; ++i)
	{
		int j;for ( j = 0; j < 8; ++j)
		{
			do
...
int CQR_Encode::CountPenalty()
{
	...
	int i;for ( i = 0; i < m_nSymbleSize; ++i)
	{
		int j;for ( j = 0; j < m_nSymbleSize - 4; ++j)
		{
			int nCount = 1;

			int k;for (k = j + 1; k < m_nSymbleSize; k++)
			{
	...
	{
		int j;for ( j = 0; j < m_nSymbleSize - 1; ++j)
		{
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		int j;for ( j = 0; j < m_nSymbleSize - 6; ++j)
		{
			if (((j < 1) ||					 (! (m_byModuleData[i][j - 1] & 0x11))) && // 明(マージン)
											 (   m_byModuleData[i][j]     & 0x11)   && // 暗
											 (! (m_byModuleData[i][j + 1] & 0x11))  && // 明
											 (   m_byModuleData[i][j + 2] & 0x11)   && // 暗
											 (   m_byModuleData[i][j + 3] & 0x11)   && // 暗
											 (   m_byModuleData[i][j + 4] & 0x11)   && // 暗
											 (! (m_byModuleData[i][j + 5] & 0x11))  && // 明
											 (   m_byModuleData[i][j + 6] & 0x11)   && // 暗
				((j >= m_nSymbleSize - 7) || (! (m_byModuleData[i][j + 7] & 0x11))))   // 明(マージン)
			{
				nPenalty += 40;
			}
		}
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		int j;for ( j = 0; j < m_nSymbleSize - 6; ++j)
		{
			if (((j < 1) ||					 (! (m_byModuleData[j - 1][i] & 0x11))) && // 明(マージン)
											 (   m_byModuleData[j]    [i] & 0x11)   && // 暗
											 (! (m_byModuleData[j + 1][i] & 0x11))  && // 明
											 (   m_byModuleData[j + 2][i] & 0x11)   && // 暗
											 (   m_byModuleData[j + 3][i] & 0x11)   && // 暗
											 (   m_byModuleData[j + 4][i] & 0x11)   && // 暗
											 (! (m_byModuleData[j + 5][i] & 0x11))  && // 明
											 (   m_byModuleData[j + 6][i] & 0x11)   && // 暗
				((j >= m_nSymbleSize - 7) || (! (m_byModuleData[j + 7][i] & 0x11))))   // 明(マージン)
			{
				nPenalty += 40;
			}
		}
